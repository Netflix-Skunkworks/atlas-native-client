...
std::unique_ptr<interpreter::TagsValuePairs> SubscriptionRegistry::GetMainMeasurements(
  ...
  auto result = std::make_unique<interpreter::TagsValuePairs>();
  ...
  for (const auto& m : all) {
    ...
    for (size_t i = 0; i < rules.size(); ++i) {
      ...
      if (query->Matches(*tagsValue)) {
        measurements_for_rule[i]->push_back(std::move(tagsValue));
        break;
      }
    }
...
SubscriptionResults SubscriptionRegistry::GetLwcMetricsForInterval(
  ...
  for (auto& s : subs) {
    auto tagsValuePairs = std::make_shared<TagsValuePairs>();
